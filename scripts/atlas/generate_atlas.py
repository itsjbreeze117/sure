#!/usr/bin/env python3
"""
Atlas generator — customized for Sure (Rails 7.2 personal finance app).
...
# ============================================================================
# CONFIGURATION — Customized for Sure (Rails 7.2)
# ============================================================================
...
IGNORE_NAMES = {
    ...
    "tmp", "log", "storage", "vendor", "public",
    ".bundle", ".cursor", ".claude",
    "vcr_cassettes", "charts", "mobile",
}

# Directory annotations for the tree
...
# Entrypoint file names to detect
ENTRYPOINT_NAMES = {
    "config.ru", "Rakefile",
}

# Entrypoint path patterns for Rails
...
# Content markers (not used for Rails, but keep for compatibility)
...
# Max tree depth (4 for Rails nested structure)
MAX_TREE_DEPTH = 4
...
def _collect_root_files() -> list[str]:
    ...
    notable_names = {
        ...
        "Gemfile.lock", "Procfile.dev", "config.ru",
    }
    files = []
    try:
...
def _extract_manual_sections(filepath: Path) -> tuple[str, str]:
    """Extract manual Router Table and Danger Zones sections from existing repo-map.md."""
    default_router = """| Task | Start Here |
|------|-----------|
| TODO | Add rows for the top tasks in this repo |"""

    default_danger = """| File/Area | Why It's Fragile |
|-----------|-----------------|
| TODO | Add fragile areas for this repo |"""

    if not filepath.exists():
        return default_router, default_danger

    content = filepath.read_text()

    # Extract Router Table section (between "## Where to Look" and "## Danger Zones")
    router_section = default_router
    router_marker = "## Where to Look for X -- Router Table"
    danger_marker = "## Danger Zones"
    if router_marker in content and danger_marker in content:
        start = content.index(router_marker) + len(router_marker)
        end = content.index(danger_marker)
        section = content[start:end].strip()
        # Find the actual table (skip comments)
        lines = section.split("\n")
        table_lines = [l for l in lines if not l.startswith("<!--") and not l.startswith("-->") and l.strip()]
        if table_lines and "TODO" not in table_lines[0]:
            router_section = "\n".join(table_lines)

    # Extract Danger Zones section (from "## Danger Zones" to end)
    danger_section = default_danger
    if danger_marker in content:
        start = content.index(danger_marker) + len(danger_marker)
    ...
    return router_section, danger_section


def generate_repo_map() -> str:
    """Generate the full repo-map.md content, preserving manual sections."""
    ...
    router_table, danger_zones = _extract_manual_sections(ATLAS_DIR / "repo-map.md")

    return f"""<!-- AUTO-GENERATED by scripts/atlas/generate_atlas.py -- do not edit manually (except Router Table and Danger Zones) -->
# Repo Map -- {REPO_NAME}
